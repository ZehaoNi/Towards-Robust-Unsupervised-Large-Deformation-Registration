...
def calculate_assd(x_seg, y_seg):
    # Convert x_seg and y_seg to binary images
    x_seg_binary = (x_seg > 0).astype(np.uint8)
    y_seg_binary = (y_seg > 0).astype(np.uint8)

    # Convert x_seg and y_seg to surface coordinates
    x_surface_coords = np.array(np.where(x_seg_binary)).T
    y_surface_coords = np.array(np.where(y_seg_binary)).T

    # Build kd-trees for fast nearest neighbor search
    x_tree = cKDTree(x_surface_coords)
    y_tree = cKDTree(y_surface_coords)

    # Calculate distances from x_surface_coords to y_surface_coords
    x2y_dist, _ = y_tree.query(x_surface_coords)
    # Calculate distances from y_surface_coords to x_surface_coords
    y2x_dist, _ = x_tree.query(y_surface_coords)

    # Calculate average symmetric surface distance (ASSD)
    assd = (np.mean(x2y_dist) + np.mean(y2x_dist)) / 2.0
...
def main():
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            assd = calculate_lpbaassd(def_out.detach().cpu().numpy()[0, 0, :, :, 40:51],
                                      y_seg.detach().cpu().numpy()[0, 0, :, :, 40:51])
            eval_assd.update(assd.item(), x.size(0))

            ssim = structural_similarity(y.detach().cpu().numpy()[0, 0, :, :, 40:51], x_def.detach().cpu().numpy()[0, 0, :, :, 40:51])
        ...
        print('assd : {} ,std: {}'.format(eval_assd.avg, eval_assd.std))
        print('ssim : {} ,std: {}'.format(eval_ssim.avg, eval_ssim.std))
