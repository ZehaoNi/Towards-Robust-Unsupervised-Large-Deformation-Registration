...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(5, 5, 6), shift_size=(0, 0, 0),
...
class TransMorph(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(mc_samples):
            x = self.dropout(x)
            prediction = self.reg_head(x)
            predictions.append(prediction.unsqueeze(0))

        predictions = torch.cat(predictions, dim=0)
        # mean_prediction = torch.mean(predictions, dim=0)
        variance_prediction = torch.var(predictions, dim=0)
        uncertainty_loss = torch.mean(variance_prediction)

        return out, flow, uncertainty_loss
